...
def keep_alive():
    ...
    port = os.getenv("PORT", "10000")  # Render sets PORT env
    ...
    with httpx.Client(timeout=10) as client:
        while True:
            ...
            try:
                resp = client.get(url)
                print(f"[Keep-Alive] {resp.status_code} - {resp.text[:50]}")
            except Exception as e:
                print(f"[Keep-Alive] Error: {e}")

# Start keep‑alive thread (daemon so it doesn't block shutdown)
...
HF_INDEX_BASE = os.environ.get(
    "ICMR_HF_INDEX_BASE",
    "https://huggingface.co/datasets/TfqDeadlox636/icrm-hitek-fulldb/resolve/main",
).rstrip("/")
